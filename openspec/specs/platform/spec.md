...
The system SHALL not depend on UI frameworks (React, Vue, Angular, Svelte, Solid, etc.) or CSS preprocessors (Sass, Less, etc.). DOM manipulation uses TypeScript with browser DOM APIs directly. Focused utility libraries (e.g., `marked` for Markdown rendering) are permitted.
...
#### Scenario: Permitted utility libraries
- **WHEN** the system needs to parse or render a specific content format (e.g., Markdown)
- **THEN** it MAY use a focused, framework-free library that performs that single task without imposing architectural constraints
